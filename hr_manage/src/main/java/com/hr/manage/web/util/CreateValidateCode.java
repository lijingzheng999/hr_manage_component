...
public class CreateValidateCode {
	...
	private void creatImage() {
		int fontWidth = (width-10) / codeCount;// 字体的宽度
		...
		this.code = str1;
		int charX = 5;
//		int charsRealWidth = fontWidth * code.length();  
//		if(this.width > fontWidth){  
//			charX = (this.width - fontWidth)/2;  
//		}  
		double radianPercent = 0D;  
		int chartY = fontHeight-1;
		for (int i = 0; i < codeCount; i++) {
			String strRand = str1.substring(i, i + 1);
			g.setColor(getRandColor(1, 155));
			g.setFont(getFont(fontHeight));
			radianPercent =  Math.PI * (random.nextInt(60)/180D);
			if(random.nextBoolean()) {
				radianPercent = -radianPercent;
			} 
			g.rotate(radianPercent, charX+5, chartY);  
			g.drawString(strRand, charX+5, chartY);  
			g.rotate(-radianPercent, charX+5 , chartY);  
			charX += fontWidth;  
			//	            g.drawString(a,x,y);
			...
			//	            g.drawString(strRand, 15*i+18, 14); 
	...
	private Font getFont(int size) {
		...
		font[0] = new Font("Fixedsys", Font.PLAIN, size);
		//font[1] = new Font("Wide Latin", Font.PLAIN, size);
		font[1] = new Font("宋体", Font.PLAIN, size);
		//font[2] = new Font("Gill Sans Ultra Bold", Font.PLAIN, size);
	...
	public void printValidateCode(HttpServletRequest request, HttpServletResponse response) throws IOException{
	            ...
	            session.setAttribute("validate_code", this.getCode());
		...
		for(int i = 0; i < 50; i++){  
			CreateValidateCode cvc = new CreateValidateCode(108,30,4,10);
			System.out.println(cvc.getCode());
			File file = new File(dir, System.currentTimeMillis()+".jpg");
			FileOutputStream fileOutputStream = new FileOutputStream(file);
			cvc.write(fileOutputStream);
		}  
	}
