...
public interface CheckWorkDetailDAO  extends GenericDAO<CheckWorkDetail,Integer>{
	...
	public static final String COLUMNS = " id,term,start_date,end_date,manufacturer,name,expatriate_unit,entry_time,attendance_days,check_work_days,overtime_days,leave_days,manager,memo,surplus_overtime_hours,annual_leave_days,surplus_annual_leave,sick_leave_days,compassionate_leave_days,more_settlement_days,is_del,update_time,create_time  ";
	    ...
	    @SQL("SELECT  " + COLUMNS + " FROM "+TABLE+" WHERE 1 = 1 " +
	            ...
	            "#if(:1.term != null  && :1.term !='') { and term = :1.term } " +
	            " and is_del=1 " )
	    CheckWorkDetail getCheckWorkDetailByName(CheckWorkDetailCondition condition);
	    
	    @SQL(" UPDATE " + TABLE +
	    		 " SET id_del=0,update_time = now() " +
				 " WHERE name= :1.name and term = :1.term and is_del=1 " )
	    int deleteCheckWorkDetailByName(CheckWorkDetailCondition condition);
}
